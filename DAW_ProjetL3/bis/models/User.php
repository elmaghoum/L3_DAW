...
class User {
    ...
    public function suggestCoursesByTags2($userId) {
        ...
        $query = $this->db->prepare("SELECT DISTINCT c.* FROM cours c WHERE $likeConditions");
        $query->execute();
    ...
    public function getUserNoteByCourseId($userId, $course_id) {
        $query = 'SELECT note.NOTE FROM note WHERE note.ID_USER = :user_id AND note.ID_QCM = :course_id';
